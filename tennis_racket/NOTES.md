...
These properties were computed by using "bodyprop.py" utility.

----------------------------------------------------------------------------------------------------------------

Note that the CAD include strings. For these ones, a density higher than the expected one, since the same
density is imposed everywhere. However, the strings has small volume, therefore we assume their mass negligible.
...
In Modelica, there are some issues regarding scaling the CAD for visualization.
For this reason, a scaled version has been generated.
